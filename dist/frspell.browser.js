...
`));
    ...
    async function Se() {
      ...
      }) : o ? a(new WebAssembly.Instance(b, X()), b) : (ie ?? (ie = e.locateFile ? e.locateFile ? e.locateFile("ort-wasm-simd-threaded.jsep.wasm", m) : m + "ort-wasm-simd-threaded.jsep.wasm" : new URL("ort-wasm-simd-threaded.jsep.wasm", import_meta.url).href), l = await (async function(f) {
        var _ = ie;
        if (!g && !z(_)) try {
...
${u}`, c = n.createShaderModule({ code: d, label: e.name });
  ...
  function makeInt64Tensor2D(rows) {
    ...
    return new Ke("int64", data, [batch, seq]);
  }
  function makeInt64Tensor1D(values) {
    ...
    return new Ke("int64", data, [values.length]);
  }
  ...
  async function createLemmaTypePredictor({
    ...
    const session = await vf.create(modelPath, {
